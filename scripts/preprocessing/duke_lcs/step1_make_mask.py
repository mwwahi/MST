...
def create_mask_for_lesion(row, path_root_data):
    ...
    uid = row["patient-id"]
    lesion_id = row.get("nodule_id", row.name)
    
    img_path = path_root_data/ uid/  f"img.nii.gz"
    ...
    try:
        ...
        out_path = path_root_data / uid/ f'{lesion_id}.nii.gz'
...
def main(csv_path, path_root_data, args):
    ...
    df = df.dropna(subset=["patient-id", "coordX", "coordY", "coordZ"])
    
    data_dir = path_root_data  
    available_images = list(data_dir.glob("*.nii.gz"))
...
### Usage:
# python /cvib2/apps/personal/wasil/lib/classification/MST/scripts/preprocessing/duke_lcs/step1_make_mask.py --num_workers 20 
# python /cvib2/apps/personal/wasil/lib/classification/MST/scripts/preprocessing/duke_lcs/step1_make_mask.py --sequential 
